...
import { initHttpParams } from '../utils/init-http-params.util';
...
export class UserApiService {
    ...
    getSavedUsersWithoutRequest(): Observable<IUser[]> {
        //only for the work of the current implementation
    ...
    getExcelByUserId(id: string): Observable<string> {
        ...
        return this.httpService
            .get<IResponseGetUsers>('', { params: { id: id } })
            .pipe(
                map(() => {
    ...
    addUser(user: INewUser): Observable<IUser> {
        return this.httpService
            .post<IResponseGetUsers>('/users/add', user)
            .pipe(map((response) => convertToUser(response.body.results[0])));
    }

    editUser(user: IUser): Observable<IUser> {
        return this.httpService
            .put<IResponseGetUsers>('/user/edit', user)
            .pipe(
